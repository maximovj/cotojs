    ...
    return (
        <div className={`flex ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex items-start ${isCurrentUser ? 'bg-blue-100' : 'bg-gray-100'} p-2 rounded-lg shadow-md max-w-xs relative`}>
                {!isCurrentUser && <img loading='lazy' src={msg.send_by.thumbnail ? `${routes.baseUrl}/${msg.send_by.thumbnail}` : default_user_thumbnail} alt='Perfil' className='w-8 h-8 rounded-full border border-gray-300 mr-2' />}
                ...
                {isCurrentUser && <img loading='lazy' src={msg.send_by.thumbnail ? `${routes.baseUrl}/${msg.send_by.thumbnail}` : default_user_thumbnail} alt='Perfil' className='w-8 h-8 rounded-full border border-gray-300 ml-2' />}
            </div>
        </div>
    );
};
